...
class Time: # object to store and manipulate time intervals
    ...
    def __gt__(self, other):
        return (self._hrs > other._hrs) or \
               (self._hrs == other._hrs and self.mins > other.mins)
...
class Train:
    ...
    def _find_closest_time(self, station, time):
        i, j = time_intervals.index(time) - 1, time_intervals.index(time) + 1
        while i >= 0 or j < len(time_intervals):
            if time_intervals[i] in station.arrival_time:
                i -= 1
            else:
                return time_intervals[i]

            if time_intervals[j] in station.arrival_time:
                j += 1
            else:
                return time_intervals[j]
        return None
...
class Route:
    ...
    def generate_route(self, intervals, stations, n):
        res = []
        while len(res) < n:
            assigned_station = ran.choice(stations)
            assigned_time = ran.choice([t for t in intervals if t not in assigned_station.arrival_time])
            res += [(assigned_station, assigned_time)]
...
def schedule_routes(trains: list,
    ...
    for t in trains:
        t.generate_route(n=n, intervals=times, stations=stations )
...
def load_text_docs():
    try:
        ...
        with open('train_colors.txt', 'r') as tc:
            ...
            tc.close()
        return time_intervals, station_names, train_colors
    except IOError:
        raise Exception('File documents not found')


time_intervals, station_names, train_colors = load_text_docs()
...
# give a train a station and a time
    # for each, ensure that stations do not overlap
