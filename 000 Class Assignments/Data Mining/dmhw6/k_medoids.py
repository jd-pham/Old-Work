...
def k_medoids(k, points: list, show_iterations=False):
	if k <= 1:
	...
	if show_iterations:
		_k_medoids(k=k, clusters=clustering, all_points=points)
	else:
		_k_medoids(k=k, clusters=clustering)



def swap_medoid(points: list, 
	old: Point, new: Point):
	new_sum = 0
	old_sum = 0
	for pt in points:
...
# determines best medoid to minimize distance between points in a cluster
def recalculate_medoids(clusters: dict):
	changed = False
	for medoid in clusters['centers']:
		
		min_dist = medoid
		for ele in clusters[medoid.color]:
			if ele == medoid:
				continue
			elif swap_medoid(points=clusters[medoid.color],
				old=min_dist, new=ele):
				min_dist = ele
				changed = True

		if not min_dist == medoid:
			clusters['centers'].remove(medoid)
			clusters['centers'].add(min_dist)

	return changed 		# true if we reassigned the medoid
	...
	for ele in clusters['centers']:
		for pt in old.values():
			if not ele.__cmp__(pt) and ele.color == pt.color:
				return True
	return False				


def _k_medoids(k, clusters: dict(), all_points = []):
	if all_points:
	...
	if 'no_group' in clusters: 
		...
		res = recalculate_medoids(clusters)
		if res:	# if there was a change, continue
			_k_medoids(k=k, clusters=clusters, all_points=all_points)
